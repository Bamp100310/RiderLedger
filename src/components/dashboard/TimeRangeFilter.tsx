...
export const TimeRangeFilter: React.FC = () => {
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <div className="bg-white dark:bg-slate-900/90 border border-slate-200/90 dark:border-white/10 rounded-2xl p-2.5 sm:p-3 shadow-xs dark:shadow-md flex items-center justify-between gap-2 transition-colors">
        {/* Botón Período Anterior */}
        <button
          onClick={() => navigateFilter(-1)}
          disabled={!isNavigable}
          className={`p-2 rounded-xl border border-slate-200/80 dark:border-white/5 transition-all flex items-center justify-center ${
            isNavigable
              ? 'bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-white dark:hover:bg-slate-700 active:scale-95'
        ...
        <div className="flex flex-col items-center justify-center min-w-0 text-center px-1">
          <div className="flex items-center gap-1.5">
            ...
            {!isCurrentPeriod && filter.range !== 'historico' && (
              <button
                ...
                className="text-[10px] bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 hover:bg-cyan-500/25 px-1.5 py-0.5 rounded-md font-bold transition-all flex items-center gap-0.5"
          ...
          <h2 className="text-sm sm:text-base font-black text-slate-900 dark:text-white truncate max-w-[280px] sm:max-w-md">
            {currentLabel}
          </h2>
        </div>
      ...
      <div className="flex items-center justify-between gap-1 p-1 bg-slate-100/90 dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/5 rounded-xl transition-colors">
        <div className="flex items-center gap-1 overflow-x-auto w-full scrollbar-none">
            ...
            return (
              <button
                ...
                className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all whitespace-nowrap text-center ${
                  isSelected
                    ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-xs font-extrabold'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-white/80 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/5'
                }`}
              >
                {r.label}
              </button>
          ...
          <button
            ...
            className={`py-1.5 px-2.5 rounded-lg text-xs font-semibold flex items-center gap-1 transition-all ${
              filter.range === 'personalizado'
                ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold shadow-xs'
                : 'text-slate-600 hover:text-slate-900 hover:bg-white/80 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/5'
            }`}
          ...
          <button
            onClick={() => triggerSync()}
            disabled={isSyncing}
            className={`py-1.5 px-2.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all text-slate-500 hover:text-cyan-600 hover:bg-white/80 dark:text-slate-400 dark:hover:text-cyan-400 dark:hover:bg-cyan-500/10 ${
      ...
      {showCustomModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl p-5 w-full max-w-xs shadow-2xl space-y-4">
            ...
            <form onSubmit={handleApplyCustom} className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowCustomModal(false)}
                  className="flex-1 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold dark:hover:bg-slate-700"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-xs font-bold shadow-md hover:from-cyan-400 hover:to-blue-500"
                >
                  Aplicar
                </button>
              </div>
